...
public abstract class Neo4jIntegrationTest {
    ...
    protected void createUsers() {
        executeCypher("CREATE USER user1 SET PASSWORD 'abc' CHANGE NOT REQUIRED");
        executeCypher("CREATE USER user2 SET PASSWORD 'abc' CHANGE NOT REQUIRED");
    }
    ...
    protected final void executeCypher(String query) {
        try (Session session = driver.session()) {
            session.run(query, Map.of());
        }
    }

    protected final List<Record> executeQuery(String query) {
        try (Session session = driver.session()) {
    ...
    protected final void createMoviesGraph() {
        executeCypher("CREATE (TheMatrix:Movie {title:'The Matrix', released:1999, tagline:'Welcome to the Real World'})\n" +
